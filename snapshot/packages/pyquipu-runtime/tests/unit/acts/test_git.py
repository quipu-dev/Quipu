...
class TestGitActs:
    ...
    def setup_git_env(self, executor: Executor, isolated_vault: Path):
        ...
        subprocess.run(["git", "config", L.user.email, "quipu@test.com"], cwd=isolated_vault, check=True)
        subprocess.run(["git", "config", L.user.name, "Quipu Bot"], cwd=isolated_vault, check=True)

    def test_git_workflow(self, executor: Executor, isolated_vault: Path, mock_runtime_bus):
        # 1. 创建文件
        target_file = isolated_vault / L.README.md
        target_file.write_text("# Test Repo", encoding="utf-8")

        # 2. Git Add
        git_add, _, _ = executor._acts["git_add"]
        ctx = ActContext(executor)
        git_add(ctx, [L.README.md])
        mock_runtime_bus.success.assert_called_with(L.acts.git.success.added, targets=[L.README.md])
        ...
        mock_runtime_bus.success.assert_called_with(L.acts.git.success.committed, message="Initial commit")
    ...
    def test_git_init_idempotent(self, executor: Executor, mock_runtime_bus):
        ...
        ctx = ActContext(executor)
        func(ctx, [])
        mock_runtime_bus.warning.assert_called_with(L.acts.git.warning.repoExists)

    def test_git_status_output_stream(self, executor: Executor, isolated_vault: Path, mock_runtime_bus):
        # 1. 制造一些状态变更
        (isolated_vault / L.untracked.txt).write_text("new file")
        ...
        mock_runtime_bus.info.assert_any_call(
            L.runtime.executor.info.executing, current=1, total=1, act_name="git_status", mode="exclusive", arg_count=0
        )
        ...
        assert L.untracked.txt in data_arg
