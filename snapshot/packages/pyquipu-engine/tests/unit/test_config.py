...
def test_config_get_defaults(work_dir: Path):
    ...
    assert config.get(L.storage.type) == "sqlite"
    # Test a non-existent key with a fallback
    assert config.get(L.nonexistent.key, "fallback") == "fallback"
    # Test a default that is None
    assert config.get(L.sync.user_id) is None
...
def test_config_load_from_file(work_dir: Path):
    ...
    assert config.get(L.sync.user_id) == "test-user-from-file"
    # Default value should still be accessible if not overridden
    assert config.get(L.sync.remote_name) == "origin"
...
def test_config_set_and_save(work_dir: Path):
    ...
    config.set(L.sync.user_id, "test-user-new")
    assert config.get(L.sync.user_id) == "test-user-new"

    # Save the configuration to disk
    config.save()

    config_file = work_dir / ".quipu" / L.config.yml
...
def test_config_nested_set_and_save(work_dir: Path):
    ...
    config.set(L.a.b.c, 123)
    config.save()

    # Verify by loading with a new instance
    new_config = ConfigManager(work_dir)
    assert new_config.get(L.a.b.c) == 123


def test_config_round_trip_consistency(work_dir: Path):
    # --- Phase 1: Create and save ---
    config1 = ConfigManager(work_dir)
    config1.set(L.storage.type, "sqlite")
    config1.set(L.sync.user_id, "round-trip-user")
    config1.set(L.sync.subscriptions, ["user-a", "user-b"])
    config1.save()

    # --- Phase 2: Load in a new instance and verify ---
    config2 = ConfigManager(work_dir)
    assert config2.get(L.storage.type) == "sqlite"
    assert config2.get(L.sync.user_id) == "round-trip-user"
    assert config2.get(L.sync.subscriptions) == ["user-a", "user-b"]
    # Verify a default value is still accessible
    assert config2.get(L.sync.remote_name) == "origin"
