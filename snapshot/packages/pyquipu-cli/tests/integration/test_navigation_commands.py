...
def test_cli_back_forward_flow(runner, populated_workspace, monkeypatch):
    ...
    monkeypatch.setattr(L.quipu.cli.commands.navigation.bus, mock_bus_nav)
    monkeypatch.setattr(L.quipu.cli.commands.helpers.bus, mock_bus_helper)

    # Initial state is B. Let's checkout to A.
    runner.invoke(app, ["checkout", hash_a[:7], "-w", str(workspace), "-f"])
    assert (workspace / L.a.txt).exists()
    assert not (workspace / L.b.txt).exists()
    ...
    mock_bus_nav.success.assert_called_with(L.navigation.back.success, short_hash=ANY)
    assert (workspace / L.b.txt).exists()
    assert not (workspace / L.a.txt).exists()
    ...
    mock_bus_nav.success.assert_called_with(L.navigation.forward.success, short_hash=ANY)
    assert (workspace / L.a.txt).exists()
    assert not (workspace / L.b.txt).exists()
